...
func GetPredictionsForStop(stopID string, apiURL string) (*Predictions, error) {
	...
	if apiURL == "" {
		apiURL = fmt.Sprintf("https://api.actransit.org/transit/stops/%s/predictions", stopID)
	}

	stopsURL, err := appendAuthToURL(apiURL, nil)
	if err != nil {
		return nil, err
	}
